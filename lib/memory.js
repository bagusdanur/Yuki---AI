...
export function saveUserArtifact(userId, artifact = {}) {
  const u = uid(userId)
  const artId = String(artifact.id || `art_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`)
  ...
  // Cek apakah artifact dengan ID ini sudah ada
  const existing = db.prepare('SELECT id, version FROM user_artifacts WHERE id = ? AND user_id = ?').get(artId, u)

  if (existing) {
    const nextVersion = (existing.version || 1) + 1
    db.prepare(`
      UPDATE user_artifacts 
      SET title = ?, content = ?, version = ?, patch_note = ?, updated_at = datetime('now')
      WHERE id = ? AND user_id = ?
    `).run(title, content, nextVersion, patchNote, artId, u)
    return { id: artId, title, type, content, version: nextVersion, patch_note: patchNote }
  }

  // Cek apakah ada artifact aktif sebelumnya dengan judul serupa untuk user ini
  const similar = db.prepare('SELECT id, version FROM user_artifacts WHERE user_id = ? AND title = ? ORDER BY updated_at DESC LIMIT 1').get(u, title)
  if (similar) {
    const nextVersion = (similar.version || 1) + 1
    db.prepare(`
      UPDATE user_artifacts 
      SET id = ?, content = ?, version = ?, patch_note = ?, updated_at = datetime('now')
      WHERE id = ? AND user_id = ?
    `).run(artId, content, nextVersion, patchNote, similar.id, u)
    return { id: artId, title, type, content, version: nextVersion, patch_note: patchNote }
  }
