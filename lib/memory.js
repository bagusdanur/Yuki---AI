...
import { summarizeProfile } from './llm.js'
...
db.exec(`CREATE TABLE IF NOT EXISTS facts (user_id TEXT NOT NULL, text TEXT NOT NULL, vec TEXT, created_at TEXT DEFAULT (datetime('now')), UNIQUE(user_id, text)); CREATE TABLE IF NOT EXISTS events (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT NOT NULL, summary TEXT NOT NULL, mood TEXT, vec TEXT, ts TEXT DEFAULT (datetime('now'))); CREATE TABLE IF NOT EXISTS emotion (user_id TEXT PRIMARY KEY, state TEXT, updated_at TEXT); CREATE TABLE IF NOT EXISTS user_profile (user_id TEXT PRIMARY KEY, profile_summary TEXT, updated_at TEXT); CREATE INDEX IF NOT EXISTS idx_facts_user ON facts(user_id); CREATE INDEX IF NOT EXISTS idx_events_user ON events(user_id);`)
...
export async function recallMemory(userId, query = '') {
  ...
  if (qvec) {
    ...
    return { profile, facts: top(facts, TOPK), events: top(events, 3) }
  }
  return { profile, facts: facts.slice(-TOPK), events: events.slice(-3) }
}
...
export function buildMemoryContext(recall) {
  ...
  if (recall?.events?.length) {
    ...
    parts.push(`Momen emosional relevan di hatimu:\n${recent}`)
  }
  return parts.join('\n\n')
}