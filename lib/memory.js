...
export async function recallMemory(userId, query = '') {
  ...
  if (qvec) {
    ...
    return { profile, facts: top(facts, TOPK), events: top(events, 3), recentEvents: recentEventsRows, chatSummary }
  }
  return { profile, facts: facts.slice(-TOPK), events: events.slice(-3), recentEvents: recentEventsRows, chatSummary }
}
...
export async function summarizeAndTrimHistory(userId) {
  ...
  // Ambil 20 pesan terlama untuk diringkas
  const oldMessages = db.prepare('SELECT id, role, content FROM chat_history WHERE user_id = ? ORDER BY created_at ASC LIMIT 20').all(u)
  ...
  const combined = existingSummary
    ...
    : newSummary
  storeChatSummary(userId, combined.slice(0, 2000)) // Batasi panjang agar tidak membengkak
