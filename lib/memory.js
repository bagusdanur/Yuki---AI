...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS agent_memories (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT NOT NULL, scope_type TEXT NOT NULL DEFAULT 'user', scope_id TEXT NOT NULL DEFAULT '', category TEXT NOT NULL, content TEXT NOT NULL, importance INTEGER NOT NULL DEFAULT 1, confidence REAL NOT NULL DEFAULT 0.75, source_message_id INTEGER, evidence_json TEXT NOT NULL DEFAULT '[]', status TEXT NOT NULL DEFAULT 'active', expires_at TEXT, created_at TEXT DEFAULT (datetime('now')), updated_at TEXT DEFAULT (datetime('now')), UNIQUE(user_id, scope_type, scope_id, category, content));
  ...
  CREATE INDEX IF NOT EXISTS idx_user_artifacts ON user_artifacts(user_id, updated_at);
`)
try { db.exec('ALTER TABLE users ADD COLUMN access_code_hash TEXT') } catch {}
db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_users_access_code_hash ON users(access_code_hash) WHERE access_code_hash IS NOT NULL')
try { db.exec('ALTER TABLE user_notes ADD COLUMN idempotency_key TEXT') } catch {}
db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_user_notes_idempotency ON user_notes(user_id, idempotency_key) WHERE idempotency_key IS NOT NULL')
...
function generateUniqueAccessCode() {
  for (let i = 0; i < 100; i++) {
    ...
    const exists = db.prepare('SELECT 1 FROM users WHERE access_code = ? OR access_code_hash = ?').get(code, hashAccessCode(code))
...
export async function registerUser(username) {
  ...
  db.prepare('INSERT INTO users (user_id, username, access_code, access_code_hash) VALUES (?, ?, NULL, ?)')
    .run(userId, username, hashAccessCode(accessCode))
...
export function getUserByAccessCode(code) {
  ...
  let row = db.prepare('SELECT user_id, username FROM users WHERE access_code_hash = ?').get(hashAccessCode(cleanCode))
  if (!row) {
    // Migrasi transparan akun lama: plaintext hanya dipakai sekali lalu dihapus.
    row = db.prepare('SELECT user_id, username FROM users WHERE access_code = ?').get(cleanCode)
    if (row) db.prepare('UPDATE users SET access_code_hash = ?, access_code = NULL WHERE user_id = ?').run(hashAccessCode(cleanCode), row.user_id)
  }
...
export function getUsernameByUserId(userId) {
  ...
  const row = db.prepare('SELECT username FROM users WHERE user_id = ?').get(uid(userId))
...
export function storeChatSummary(userId, summary) {
...
export function deleteUserData(userId) {
  const u = uid(userId)
  const tables = ['facts', 'events', 'emotion', 'user_profile', 'chat_history', 'chat_summary', 'structured_memory', 'relationship_events', 'response_feedback', 'comic_bookmarks', 'user_notes', 'user_artifacts', 'scheduled_tasks', 'self_improvements', 'agent_workflows', 'users']
  db.exec('BEGIN')
  try {
    const existing = new Set(db.prepare("SELECT name FROM sqlite_master WHERE type = 'table'").all().map(row => row.name))
    for (const table of tables) if (existing.has(table)) db.prepare(`DELETE FROM ${table} WHERE user_id = ?`).run(u)
    db.exec('COMMIT')
...
export function saveUserNote(userId, { title, content, category = 'general', idempotencyKey = '' }) {
  const u = uid(userId)
  ...
  const key = String(idempotencyKey || crypto.createHash('sha256').update(`${u}:${cleanTitle}:${cleanContent}:${cleanCat}:${new Date().toISOString().slice(0, 10)}`).digest('hex')).slice(0, 120)
  const existing = db.prepare(`SELECT id, title, content, category, created_at AS createdAt, updated_at AS updatedAt
    FROM user_notes WHERE user_id = ? AND idempotency_key = ?`).get(u, key)
  if (existing) return { ...existing, idempotent: true }

  const info = db.prepare(`
    INSERT INTO user_notes (user_id, title, content, category, idempotency_key, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, datetime('now'), datetime('now'))
  `).run(u, cleanTitle, cleanContent, cleanCat, key)

  return db.prepare(`SELECT id, title, content, category, created_at AS createdAt, updated_at AS updatedAt
    FROM user_notes WHERE id = ? AND user_id = ?`).get(Number(info.lastInsertRowid), u)
}

export function getUserNoteById(userId, noteId) {
  return db.prepare(`SELECT id, title, content, category, created_at AS createdAt, updated_at AS updatedAt
    FROM user_notes WHERE user_id = ? AND id = ?`).get(uid(userId), Number(noteId)) || null
}
...
export function getAdminStats() {
  ...
  return { users, messages, activeToday, feedbackPositive, feedbackNegative, averageBond: bonds.length ? bonds.reduce((a, b) => a + b, 0) / bonds.length : 0, recent, appData: getAppDataSummary() }
}

// Ringkasan isi database untuk dashboard admin (label ramah + jumlah baris).
export const APP_DATA_LABELS = {
  chat_history: 'Riwayat chat',
  users: 'Pengguna',
  facts: 'Fakta ingatan',
  structured_memory: 'Memori terstruktur',
  agent_memories: 'Memori agent',
  events: 'Peristiwa',
  relationship_events: 'Peristiwa hubungan',
  emotion: 'Emosi / bond',
  agent_runs: 'Agent runs',
  agent_run_steps: 'Langkah agent',
  scheduled_tasks: 'Jadwal tersimpan',
  scheduled_occurrences: 'Riwayat jadwal',
  comic_bookmarks: 'Bookmark komik',
  user_artifacts: 'Artifact user',
  response_feedback: 'Feedback respons',
  self_improvements: 'Self-improvement',
  chat_summary: 'Ringkasan chat',
  user_profile: 'Profil user',
  user_notes: 'Catatan user',
  push_subscriptions: 'Langganan push'
}

export function getAppDataSummary() {
  try {
    const names = db.prepare("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").all().map(row => row.name)
    const tables = []
    for (const name of names) {
      if (name.startsWith('sqlite_') || name.includes('_fts') || name.includes('_vec')) continue
      try {
        const count = Number(db.prepare(`SELECT COUNT(*) AS value FROM "${name}"`).get()?.value || 0)
        tables.push({ name, label: APP_DATA_LABELS[name] || name, count })
      } catch { /* lewati tabel virtual/tidak standar */ }
    }
    tables.sort((a, b) => b.count - a.count)
    const stat = statSyncSafe(FILE)
    return { file: FILE, sizeBytes: stat, tables }
  } catch (error) {
    return { file: FILE, sizeBytes: 0, tables: [] }
  }
}
...
export function getChatHistory(userId) {
  const rows = db.prepare('SELECT id AS messageId, role, content FROM chat_history WHERE user_id = ? ORDER BY created_at DESC LIMIT 40')
    .all(uid(userId))
  return rows.reverse()
}

export function getScheduledReminderMessages(userId, afterId = 0) {
  return db.prepare(`SELECT id AS messageId, role, content, created_at AS createdAt
    FROM chat_history
    WHERE user_id = ? AND id > ? AND role = 'assistant' AND (content LIKE '*[Pengingat Terjadwal:%' OR content LIKE '[YUKI_REMINDER]%')
    ORDER BY id ASC LIMIT 40`).all(uid(userId), Math.max(0, Number(afterId) || 0))
    .map(row => ({ ...row, content: row.content.replace(/^\[YUKI_REMINDER\]\s*/, '') }))
}
...
export async function recallMemory(userId, query = '') {
  ...
  const scoped = db.prepare(`SELECT scope_type AS scopeType, scope_id AS scopeId, category, content, status, confidence FROM agent_memories
    WHERE user_id = ? AND status = 'active' AND (expires_at IS NULL OR expires_at > datetime('now'))
    ORDER BY importance DESC, confidence DESC, updated_at DESC LIMIT 12`).all(u)
  const legacy = db.prepare(`SELECT 'user' AS scopeType, '' AS scopeId, category, content, status, 0.7 AS confidence FROM structured_memory WHERE user_id = ? ORDER BY CASE status WHEN 'active' THEN 0 ELSE 1 END, importance DESC, updated_at DESC LIMIT 10`).all(u)
  const structured = [...scoped, ...legacy.filter(old => !scoped.some(item => item.category === old.category && item.content === old.content))].slice(0, 12)
...
export function buildMemoryContext(recall) {
  ...
  if (recall?.structured?.length) {
    const labels = { preference: 'Preferensi', habit: 'Kebiasaan', promise: 'Janji penting', relationship: 'Orang penting', open_loop: 'Belum selesai', technical: 'Pelajaran terverifikasi' }
    parts.push(`Memori terstruktur dan scoped (jangan mengarang detail di luar ini):\n${recall.structured.map((item) => `- [${item.scopeType || 'user'}${item.scopeId ? `:${item.scopeId}` : ''}; ${labels[item.category] || item.category}; ${item.status}] ${item.content}`).join('\n')}`)
  }
...
// === PERSISTENT ARTIFACT WORKSPACE & FILE MANAGEMENT (Hermes-Grade) ===

export function saveUserArtifact(userId, artifact = {}) {
  const u = uid(userId)
  let artId = String(artifact.id || '').trim()
  const title = String(artifact.title || 'Untitled Artifact').trim()
  const type = String(artifact.type || 'html').trim()
  const content = String(artifact.content || '').trim()
  const patchNote = String(artifact.patch_note || artifact.summary || '').trim()

  if (!content) return null

  // 1. Cek apakah artifact dengan ID ini sudah ada
  let existing = artId ? db.prepare('SELECT id, version, title FROM user_artifacts WHERE id = ? AND user_id = ?').get(artId, u) : null

  // 2. Jika tidak ditemukan dengan ID yang sama, cek apakah ada file aktif sebelumnya untuk user ini
  if (!existing) {
    const latest = db.prepare('SELECT id, version, title FROM user_artifacts WHERE user_id = ? ORDER BY updated_at DESC LIMIT 1').get(u)
    if (latest) {
      const isSameTitle = latest.title.toLowerCase().trim() === title.toLowerCase().trim()
      const isRelatedTitle = title.toLowerCase().includes(latest.title.toLowerCase().slice(0, 10)) || latest.title.toLowerCase().includes(title.toLowerCase().slice(0, 10))
      if (isSameTitle || isRelatedTitle) {
        existing = latest
        artId = latest.id
      }
    }
  ...
  if (existing) {
    ...
    db.prepare(`
      ...
      SET title = ?, type = ?, content = ?, version = ?, patch_note = ?, updated_at = datetime('now')
      WHERE id = ? AND user_id = ?
    `).run(title, type, content, nextVersion, patchNote, existing.id, u)
    return { id: existing.id, title, type, content, version: nextVersion, patch_note: patchNote }
  }

  if (!artId) {
    artId = `art_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`
  }

  // Insert artifact baru
  db.prepare(`
    INSERT INTO user_artifacts (id, user_id, title, type, content, version, patch_note)
    VALUES (?, ?, ?, ?, ?, 1, ?)
  `).run(artId, u, title, type, content, patchNote)

  return { id: artId, title, type, content, version: 1, patch_note: patchNote }
}

export function getLatestUserArtifact(userId) {
  const u = uid(userId)
  const row = db.prepare('SELECT id, title, type, content, version, patch_note, created_at, updated_at FROM user_artifacts WHERE user_id = ? ORDER BY updated_at DESC LIMIT 1').get(u)
  return row || null
}

export function getUserArtifactById(userId, artifactId) {
  const u = uid(userId)
  const row = db.prepare('SELECT id, title, type, content, version, patch_note, created_at, updated_at FROM user_artifacts WHERE user_id = ? AND id = ?').get(u, artifactId)
  return row || null
}

export function listUserArtifacts(userId) {
  const u = uid(userId)
  return db.prepare('SELECT id, title, type, version, patch_note, created_at, updated_at FROM user_artifacts WHERE user_id = ? ORDER BY updated_at DESC LIMIT 20').all(u)
}

// Diekspor untuk kebutuhan dashboard admin (read-only browsing).
