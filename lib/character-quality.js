...
export function validateCharacterReply(reply = '', recentAssistant = [], target = responseTarget(''), memoryContext = '') {
  const issues = []
  ...
  if (/[,;:\-–—]\s*$/.test(clean) || /\b(dan|atau|karena|tapi|kalau|yang|untuk)\s*$/i.test(clean)) issues.push('respons tampak terpotong')
