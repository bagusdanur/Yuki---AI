...
export async function callLLM(messages, options = {}) {
  const callProvider = async (provider) => {
    ...
    if (Array.isArray(options.tools) && options.tools.length > 0) {
      ...
      if (options.tool_choice) requestPayload.tool_choice = options.tool_choice
    }
    ...
    console.info(`[llm] provider=${provider.name} model=${provider.model} latency_ms=${Date.now() - startedAt} finish=${choice?.finish_reason || 'unknown'} tool_calls=${toolCalls?.length || 0} tokens=${completion.usage?.total_tokens ?? 'n/a'}`)
    if (['length', 'max_tokens'].includes(choice?.finish_reason)) throw new Error(`${provider.name} response truncated by token limit`)
    if (!toolCalls?.length && (typeof content !== 'string' || !content.trim())) throw new Error(`${provider.name} returned an empty response`)
    return { content: content || '', message, tool_calls: toolCalls || [] }
  }
  let lastError
