...
function buildSystemPromptStatic() {
  const prompts = [
    ...
    */
    ...
    `Gestur bersifat OPSIONAL (sekitar satu dari tiga balasan), ditulis dalam *bintang* hanya jika menambah makna emosional. Variasikan dan jangan gunakan gestur sebagai pembuka otomatis. Di BARIS PALING AKHIR, tetap tulis [emosi: X] dengan X salah satu dari: tenang, senang, ceria, malu, sayang/manja, sedih, kesal, cemas, kecewa, lesu, cemburu.`
  ]
  return prompts.join('\n')
}

// 2) Bagian DINAMIS (Konteks obrolan saat ini) -> Disisipkan sebelum pesan terakhir
function buildSystemPromptDynamic({ emotionDirective = '', memoryContext = '', comicContext = '', isIdle = false, bondName = 'orang asing', retryReason = '', isSerious = false, userMsgLength = 0, shouldAskQuestion = false, sessionTurns = 0, responseStyle = '', recentAssistant = [] } = {}) {
  const prompts = []
...
async function callLLM(messages) {
  const callProvider = async (provider) => {
    ...
    console.info(`[llm] provider=${provider.name} model=${provider.model} latency_ms=${Date.now() - startedAt} finish=${choice?.finish_reason || 'unknown'} tokens=${completion.usage?.total_tokens ?? 'n/a'}`)
    if (['length', 'max_tokens'].includes(choice?.finish_reason)) throw new Error(`${provider.name} response truncated by token limit`)
