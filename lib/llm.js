...
import { createRunBudget, routeProviderRequest } from './provider-router.js'
import { runtimeProviders } from './provider-settings.js'

const REQUEST_TIMEOUT_MS = 45_000
/* Legacy static catalog retained only as documentation; runtime uses provider-settings. */
const PROVIDERS = {
  nebula: {
    id: 'nebula',
    ...
    inputCostPerMillion: Number(process.env.NEBULA_INPUT_COST_PER_MILLION || 0),
    outputCostPerMillion: Number(process.env.NEBULA_OUTPUT_COST_PER_MILLION || 0)
  },
  nebulaTerra: {
    id: 'nebulaTerra',
    ...
    inputCostPerMillion: Number(process.env.GEMINI_INPUT_COST_PER_MILLION || 0),
    outputCostPerMillion: Number(process.env.GEMINI_OUTPUT_COST_PER_MILLION || 0)
  },
  nineRouter: {
    name: '9Router (Gemini Flash)',
...
export async function callLLM(messages, options = {}) {
  const callProvider = async (provider, customMessages = messages, customOptions = options) => {
    ...
    return { content, message, tool_calls: toolCalls, finishReason: choice?.finish_reason || 'unknown', usage: completion.usage || {} }
  }
  return routeProviderRequest({
    providers: runtimeProviders(),
    invoke: provider => callProvider(provider),
    budget: options.budget || createRunBudget()
  })
}
