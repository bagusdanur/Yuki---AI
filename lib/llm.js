...
function sanitizeMessagesForFilter(msgs = [], level = 'soft') {
  const SOFT_WORDS = /(tsundere|manja|sayang|cinta|kekasih|peluk|cium|dere-dere|ngambek)/gi
...
export async function callLLM(messages, options = {}) {
  ...
  try {
    ...
    // Retry tanpa tools jika malformed function call
    ...
    if (isContentBlocked(error)) {
      ...
      try {
        ...
        if (isContentBlocked(e1)) {
          ...
          // 4b. Sanitasi hard (lebih agresif) + strip tools
          try {
            const hardClean = sanitizeMessagesForFilter(messages, 'hard')
            const res2 = await callProvider(PROVIDERS.agentRouter, hardClean, { ...options, tools: undefined })
