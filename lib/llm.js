...
export function buildToolCompatibilityMessages(messages = [], tools = []) {
  ...
  return [{
    role: 'system',
    content: `MODE KOMPATIBILITAS TOOL: Native tool-calling provider sedang tidak tersedia. Jika tugas membutuhkan tool, balas HANYA dengan satu atau beberapa tag berikut tanpa uraian lain:\n<tool_call>{"name":"NAMA_TOOL","arguments":{}}</tool_call>\nGunakan hanya tool dan schema di katalog ini:\n${JSON.stringify(catalog)}`
  }, ...messages]
}
...
export async function callLLM(messages, options = {}) {
  const callProvider = async (provider, customMessages = messages, customOptions = options) => {
    ...
    const requestPayload = {
      ...
      max_tokens: customOptions.max_tokens ?? 16384
    }
    ...
    if (choice?.finish_reason === 'malformed_function_call') {
      if (content && content.trim()) {
        return { content, message, tool_calls: [], finishReason: choice?.finish_reason || 'malformed_function_call' }
      }
    ...
    return { content, message, tool_calls: toolCalls, finishReason: choice?.finish_reason || 'unknown' }
  }

  let lastError
  const isContentBlocked = (err) => String(err?.message || '').includes('content-blocked') || err?.status === 400

  // 1. Percobaan Nebula Primary (GPT-5.6 Luna - fastest 1.2s)
  try {
    const res = await callProvider(PROVIDERS.nebula)
    return { ...res, provider: PROVIDERS.nebula }
  } catch (error) {
    lastError = error
    console.warn(`[llm] Nebula Primary gagal: ${error.message}`)
  }

  // 1b. Percobaan Nebula Terra (GPT-5.6 Terra - backup 1.5s)
  try {
    const res = await callProvider(PROVIDERS.nebulaTerra)
    return { ...res, provider: PROVIDERS.nebulaTerra }
  } catch (error) {
    lastError = error
    console.warn(`[llm] Nebula Terra gagal: ${error.message}`)
  }

  // Gateway OpenAI-compatible tertentu gagal secara internal ketika payload
  // memuat native `tools`. Runner juga mendukung <tool_call> tekstual, sehingga
  // Agent tetap bisa mengeksekusi tool tanpa mengarang status sukses.
  if (Array.isArray(options.tools) && options.tools.length > 0) {
    const compatibilityMessages = buildToolCompatibilityMessages(messages, options.tools)
    const compatibilityOptions = { ...options, tools: [], tool_choice: undefined }
    for (const provider of [PROVIDERS.nebula, PROVIDERS.nebulaTerra]) {
      try {
        const res = await callProvider(provider, compatibilityMessages, compatibilityOptions)
        console.warn(`[llm] Native tool-calling gagal; memakai compatibility mode provider=${provider.name}`)
        return { ...res, provider, compatibilityMode: true }
      } catch (error) {
        lastError = error
        console.warn(`[llm] Compatibility mode ${provider.name} gagal: ${error.message}`)
      }
    }
  }

  // Semua provider LLM gagal (Nebula Primary + Terra)
