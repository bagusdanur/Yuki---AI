...
async function callLLM(messages) {
  const callProvider = async (provider) => {
    const completion = await provider.client.chat.completions.create({ model: provider.model, messages, stream: false, temperature: 0.72, max_tokens: 350 })
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    catch (error) { lastError = error; console.warn(`[llm] AgentRouter attempt ${attempt}/2 gagal: ${error.message}`) }
  }
  try { return { content: await callProvider(PROVIDERS.nineRouter), provider: PROVIDERS.nineRouter } }
  catch (error) { throw new Error(`AgentRouter gagal setelah 2 percobaan; 9Router juga gagal: ${error.message}`, { cause: lastError }) }
/*
