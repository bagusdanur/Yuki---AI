...
export async function callLLM(messages, options = {}) {
  const callProvider = async (provider, customMessages = messages, customOptions = options) => {
    ...
    const requestPayload = {
      ...
      max_tokens: customOptions.max_tokens ?? 4096
    }
    ...
    if (choice?.finish_reason === 'malformed_function_call' && !content && !toolCalls.length) {
  ...
  // Percobaan 1 & 2: 9Router (Gemini Agent)
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      ...
      lastError = error
      console.warn(`[llm] 9Router attempt ${attempt}/2 gagal: ${error.message}`)
      // Jika kegagalan adalah malformed_function_call, coba retry sekali tanpa tools
  ...
  // Percobaan Fallback: AgentRouter
  try {
    const res = await callProvider(PROVIDERS.agentRouter)
    return { ...res, provider: PROVIDERS.agentRouter }
  } catch (error) {
    // Jika AgentRouter terblokir content-filter (400 content-blocked), bersihkan instruksi sensitif dan coba lagi
    if (String(error.message || '').includes('content-blocked') || error.status === 400) {
      console.warn('[llm] AgentRouter terkena filter konten, mencoba request dengan pesan tersanitasi...')
      const sanitizedMessages = messages.map(m => {
        if (m.role !== 'system') return m
        const cleanContent = String(m.content || '')
          .replace(/(tsundere|manja|sayang|cinta|kekasih|peluk|cium|dere-dere|ngambek)/gi, 'akrab')
        return { ...m, content: cleanContent }
      })
      try {
        const cleanRes = await callProvider(PROVIDERS.agentRouter, sanitizedMessages)
        return { ...cleanRes, provider: PROVIDERS.agentRouter }
      } catch (cleanErr) {
        throw new Error(`9Router & AgentRouter gagal merespon: ${cleanErr.message}`, { cause: lastError })
      }
    }
    throw new Error(`9Router gagal setelah 2 percobaan; AgentRouter juga gagal: ${error.message}`, { cause: lastError })
  }
}
