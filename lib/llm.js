...
export async function callLLM(messages, options = {}) {
  ...
  // 2. Percobaan 9Router Flash (Model cepat tanpa filter/rate-limit)
  try {
    const res = await callProvider(PROVIDERS.nineRouterFlash)
    return { ...res, provider: PROVIDERS.nineRouterFlash }
  } catch (error) {
    lastError = error
    console.warn(`[llm] 9Router Flash gagal: ${error.message}`)
  }

  // 3. Percobaan 9Router Pro
  try {
