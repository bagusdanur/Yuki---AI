...
function buildSystemPromptDynamic({ emotionDirective = '', memoryContext = '', comicContext = '', isIdle = false, bondName = 'orang asing', retryReason = '', isSerious = false, userMsgLength = 0, shouldAskQuestion = false, sessionTurns = 0 } = {}) {
  ...
  // K-3: Kesadaran waktu nyata — Yuki tahu hari & jam saat ini
  const _now = new Date()
  const _days = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu']
  const _hari = _days[_now.getDay()]
  const _jam = _now.getHours()
  const _menit = _now.getMinutes().toString().padStart(2, '0')
  const _timeOfDay = _jam >= 5 && _jam < 12 ? 'pagi' : _jam >= 12 && _jam < 15 ? 'siang' : _jam >= 15 && _jam < 18 ? 'sore' : _jam >= 18 && _jam < 22 ? 'malam' : 'dini hari'
  prompts.push(`Waktu nyata sekarang: ${_hari}, ${_jam.toString().padStart(2, '0')}:${_menit} (${_timeOfDay}). Gunakan info ini secara alami jika relevan — misalnya menyapa sesuai waktu, atau berkomentar singkat soal waktu kalau konteksnya pas.`)
  ...
  // Y-6: Varians mood harian deterministik — Yuki sedikit berbeda tiap hari
  const _dailyHint = (() => {
    const _d = new Date()
    const _dseed = _d.getFullYear() * 10000 + (_d.getMonth() + 1) * 100 + _d.getDate()
