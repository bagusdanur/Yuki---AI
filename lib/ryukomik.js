...
function detectFormat(item = {}) {
  const haystack = `${item.title || ''} ${item.type || ''} ${item.type_genre || ''} ${item.genre || ''} ${item.link || ''}`.toLowerCase()
  if (haystack.includes('manhwa')) return 'MANHWA'
  if (haystack.includes('manhua')) return 'MANHUA'
  if (haystack.includes('manga')) return 'MANGA'
  return 'KOMIK'
}
...
function normalize(x = {}) {
  ...
  return {
    ...
    type: String(x.genre || x.type_genre || '').trim(),
    chapter: String(x.chapter_terbaru || x.update || '').trim(),
    score: rawScore,
    format,
    image: x.image || ''
  }
