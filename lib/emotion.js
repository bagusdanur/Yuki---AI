...
export class Emotion {
  constructor(state = {}) {
    ...
    this.relationship = { positive: 0, conflict: 0, repair: 0, ...(state.relationship || {}) }
    this.episode = state.episode && typeof state.episode === 'object' ? state.episode : null
  ...
  updateFromLLMMood(llmMood) {
    ...
    switch (llmMood) {
      ...
      case 'lesu':
        ...
        break
      case 'tenang':
        if (!this.episode || this.episode.turnsRemaining <= 0) {
          m.anger = base.anger
    ...
    if (llmMood && llmMood !== 'tenang') this.rememberEpisode(llmMood, this.episode?.cause || 'dinamika percakapan')
  }

  rememberEpisode(label, cause = '') {
    const duration = { kesal: 3, kecewa: 4, sedih: 3, cemburu: 2, cemas: 2, malu: 1, 'sayang/manja': 2, senang: 2, ceria: 2 }[label] || 1
    const intensity = Math.max(this.mood.anger || 0, this.mood.sadness || 0, this.mood.fear || 0, this.mood.fluster || 0, this.mood.joy || 0)
    this.episode = { label, cause: String(cause).slice(0, 160), intensity, turnsRemaining: duration, startedAt: Date.now() }
  }
  ...
  decay({ perTurn = true } = {}) {
    ...
    const activeEpisode = this.episode && this.episode.turnsRemaining > 0
    const turnRate = perTurn ? (activeEpisode ? 0.12 : 0.28) : 0
    ...
    if (perTurn && activeEpisode) {
      this.episode.turnsRemaining -= 1
      this.episode.intensity = clamp((this.episode.intensity || 0) * 0.82)
      if (this.episode.turnsRemaining <= 0) this.episode = null
    }
  }
  ...
  react(text = '') {
    ...
    if (isSoothe) {
      ...
      this.relationship.repair += wasAngry || wasSad ? 1 : 0.25
    }
    ...
    if (bondDelta > 0.15) this.relationship.positive += Math.min(1, bondDelta)
    this.interactions += 1
    if (normalizedInput) this.recentInputs = [...this.recentInputs, normalizedInput].slice(-8)
    const currentLabel = this.label()
    if (['kesal', 'kecewa', 'sedih', 'cemburu', 'cemas', 'malu'].includes(currentLabel)) this.rememberEpisode(currentLabel, text)
    return { mood: currentLabel, bond: this.bondLevel() }
  }
  ...
  directive() {
    ...
    if (this.episode?.turnsRemaining > 0) lines.push(`Emosi ini masih tersisa sekitar ${this.episode.turnsRemaining} giliran karena: "${this.episode.cause}". Emosi boleh melunak, tetapi jangan berubah total tanpa pemicu yang masuk akal.`)
    if (mood === 'malu') {
