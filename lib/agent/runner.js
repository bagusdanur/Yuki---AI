...
// 🛡️ PERSISTENT ARTIFACT WORKSPACE & INJEKSI KODE AKTIF (Hermes-Grade):
// Optimalkan riwayat chat lampau sambil menjamin LLM selalu memiliki akses penuh ke kode file game/aplikasi aktif saat ini.
function prepareAgentMessages(messages = [], activeArtifact = null) {
  const recent = messages.slice(-10)
  
  // 1. Temukan kode artifact terbaru dari database persistent workspace ATAU riwayat pesan
  let latestArtifactCode = activeArtifact ? activeArtifact.content : null
  let latestArtifactTitle = activeArtifact ? activeArtifact.title : 'Active Canvas App'
  ...
  if (!latestArtifactCode) {
    for (let i = recent.length - 1; i >= 0; i--) {
      ...
      if (m.role === 'assistant') {
        ...
        if (match) {
          latestArtifactCode = match[1].trim()
          break
        }
      }
    }
  ...
  return recent.map((msg) => {
    ...
    // c. Jika ini pesan asisten terakhir dan ada artifact aktif di workspace, sertakan kodenya sebagai acuan iterasi Hermes
    if (msg.role === 'assistant' && latestArtifactCode && !preservedLatest) {
      preservedLatest = true
      content += `\n\n[FILE ARTIFACT AKTIF DI WORKSPACE USER (Hermes Workspace v${latestArtifactVersion})]:\n- Judul: "${latestArtifactTitle}"\n- Kode Sumber:\n\`\`\`html\n${latestArtifactCode}\n\`\`\`\n*(PANDUAN ITERASI HERMES: Kode di atas adalah file yang sedang dibuka dan dimainkan user saat ini. Jika user meminta 'tambahkan level', 'perbaiki bug', atau 'tambah fitur', BACA KODE DI ATAS dan pertahankan seluruh variabel kontrol/game/audio yang ada, lalu modifikasi atau tambahkan level baru (misal Level 4 dan Level 5) langsung pada kode tersebut! JANGAN MEMBUAT GAME BARU DARI NOL!)*`
    }
...
export async function runAgent({
  ...
  for (let turn = 1; turn <= maxTurns; turn++) {
    ...
    const toolPromises = parsedCalls.map(async ({ tc, toolName, args }) => {
      ...
      // Jalankan tool dengan timeout per-tool 18 detik dan context userId + activeArtifactId
      let execResult
      try {
        execResult = await executeToolWithTimeout(toolName, args, { userId, activeArtifactId: activeArtifact?.id })
      } catch (timeoutErr) {
  ...
  // Simpan/sinkronkan artifact yang dihasilkan ke user_artifacts database
  if (userId && artifactResults.length > 0) {
    for (const art of artifactResults) {
      try {
        saveUserArtifact(userId, art)
      } catch (err) {
        console.warn('[runner] Gagal simpan artifact ke user_artifacts:', err.message)
      }
    }
  }
