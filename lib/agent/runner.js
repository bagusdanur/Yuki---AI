...
export function selectAgentToolNames(text = '') {
  ...
  if (/\b(todo|catatan|note|agenda|ingatkan\w*|pengingat\w*|jadwal\w*|schedule\w*|reminder\w*|hapus catatan|batalkan jadwal)\b/.test(query)) add(['add_user_note', 'list_user_notes', 'delete_user_note', 'schedule_task', 'list_scheduled_tasks', 'cancel_scheduled_task'])
...
export function buildDeterministicSchedulerReport(steps = []) {
  const step = [...steps].reverse().find(item => ['schedule_task', 'list_scheduled_tasks', 'cancel_scheduled_task'].includes(item.tool) && item.status === 'done')
  if (!step) return ''
...
export function guardGroundedFinalResponse(text = '', toolContracts = []) {
  const value = String(text || '').trim()
  const makesSideEffectClaim = /\b(berhasil|sudah|telah)\b[\s\S]{0,90}\b(disimpan|dibuat|dijalankan|dihapus|dijadwalkan|diperbarui|diubah)\b/i.test(value)
  ...
  const requiredKinds = []
  if (/\b(disimpan|dibuat|diperbarui|diubah)\b/i.test(value)) requiredKinds.push(new Set(['file', 'record', 'artifact']))
...
export async function runAgent({
  ...
  const taskIntent = resolveAgentTaskIntent(userRequest, contextualRequest, Boolean(activeArtifact))
  const selectedToolNames = selectAgentToolsForConversation(messages)
  ...
  if (!awaitingApproval && changedWorkspacePaths.size > 0) {
  ...
  if (!awaitingApproval) emitProgress(finalStep)
  if (!awaitingApproval && !finalContent.trim()) {
  ...
  const schedulerReport = buildDeterministicSchedulerReport(steps)
  ...
  cleanReply = guardGroundedFinalResponse(cleanReply, toolContracts)
  ...
  return {
    ...
    mode: 'agent',
    steps,
    workflowState: awaitingApproval ? 'awaiting_approval' : steps.some(step => step.status === 'error') ? 'partially_succeeded' : 'succeeded',
    awaitingApproval,
