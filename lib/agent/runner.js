...
function prepareAgentMessages(messages = []) {
  ...
  // 1. Temukan kode artifact terbaru dari riwayat pesan untuk dijadikan acuan bug fix
  ...
  return recent.map((msg, index) => {
    ...
    // b. Jika ini adalah pesan asisten yang memuat artifact terbaru, pertahankan kodenya agar bisa diperbaiki
    if (msg.role === 'assistant' && latestArtifactCode && content.includes(latestArtifactCode) && !preservedLatest) {
      preservedLatest = true
      // Tandai dengan jelas sebagai kode acuan iterasi
      content = content.replace(latestArtifactCode, `\n\n[KONTEKS ARTIFACT KODE AKTIF SEBELUMNYA]:\n${latestArtifactCode}\n*(Instruksi: Jika user meminta perbaikan bug/revisi, modifikasi dan perbaiki kode di atas daripada membuat baru dari nol)*\n\n`)
    } else {
      // Untuk artifact versi lebih lama, ringkas agar hemat token
      content = content.replace(/```(?:html|xml|svg)\s*\n[\s\S]*?```/gi, '[Lampiran File Artifact Versi Sebelumnya]')
    }

    // c. Batasi panjang pesan asisten lampau jika terlalu panjang (kecuali yang memuat kode aktif)
    if (msg.role === 'assistant' && content.length > 25000) {
      content = content.slice(0, 25000) + '... [output dipadatkan]'
    }
