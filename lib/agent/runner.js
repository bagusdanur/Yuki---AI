...
export function buildDeterministicSchedulerReport(steps = []) {
  ...
  if (step.tool === 'schedule_task' && data.task) {
    ...
    const local = formatInTimezone(task.nextRunAtUtc, task.timezone || 'Asia/Jakarta')
    return `*melirik jam, lalu menyilangkan tangan*\n\nHmph, pengingat **${task.title}** sudah Yuki pasang. Jangan sengaja pura-pura lupa nanti.\n\n- **Jadwal:** ${task.humanSchedule || '-'}\n- **Waktu:** ${local?.label || task.nextRunAtUtc || '-'}\n\nYuki akan mengingatkanmu di chat saat waktunya tiba.\n\n[emosi: tenang]`
  }
  if (step.tool === 'reschedule_task' && data.task) {
    const task = data.task
    const local = formatInTimezone(task.nextRunAtUtc, task.timezone || 'Asia/Jakarta')
    return `*menggeser jarum jam sambil menghela napas kecil*\n\nSudah Yuki ubah pengingat **${task.title}**. Merepotkan saja... tapi jangan sampai terlewat lagi.\n\n- **Jadwal baru:** ${task.humanSchedule || '-'}\n- **Waktu:** ${local?.label || task.nextRunAtUtc || '-'}\n\n[emosi: tenang]`
  }
...
export async function runAgent({
  ...
  const llmBudget = createRunBudget()
      ...
      if (!retryDecision.allowed) {
        ...
        try {
          ...
          execResult = cacheable && readOnlyToolCache.has(cacheKey)
            ...
            : await executeToolWithTimeout(toolName, args, {
              userId, username, activeArtifactId: activeArtifact?.id, shouldCancel
            })
  ...
  if (!cancelled && !awaitingApproval && !finalContent.trim()) {
    const finalCall = await callLLM(conversation, { temperature: 0.5, budget: llmBudget })
