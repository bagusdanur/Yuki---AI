...
const DEFAULT_LIMITS = Object.freeze({ timeoutMs: 10_000, outputBytes: 16_000, memoryMb: 256, virtualMemoryMb: 2048, cpuSeconds: 8, processes: 8, fileBytes: 1_048_576 })
...
export async function runSandboxedNodeTest({ entry, workspaceRoot, limits = {} }) {
  ...
  if (process.platform === 'linux' && existsSync('/usr/bin/prlimit')) {
    ...
    args = [`--as=${policy.virtualMemoryMb * 1024 * 1024}`, `--cpu=${policy.cpuSeconds}`, `--nproc=${policy.processes}`, `--fsize=${policy.fileBytes}`, '--', process.execPath, ...nodeArgs]
