...
function minimizeToolAllowlist(names, text, intent) {
  ...
  const value = String(text).toLowerCase()
  if (intent === 'analyze') {
    for (const name of [...selected]) if (/^(?:create_|replace_|patch_|update_|delete_|rollback_|schedule_|cancel_)/.test(name)) selected.delete(name)
  }
  ...
  if (/\b(jadwal|schedule|reminder|pengingat)\b/.test(value)) {
    const desired = /\b(batal|batalkan|cancel|hapus)\b/.test(value) ? 'cancel_scheduled_task'
      : /\b(list|daftar|tampil|lihat|semua)\b/.test(value) ? 'list_scheduled_tasks' : 'schedule_task'
    for (const name of ['schedule_task', 'list_scheduled_tasks', 'cancel_scheduled_task']) if (name !== desired) selected.delete(name)
  }
...
export function buildExecutionPlan(messages = [], { selectTools, hasActiveArtifact = false, maxSteps = 10 } = {}) {
  ...
  const mutating = toolAllowlist.some(name => /^(?:create_|replace_|patch_|update_|delete_|schedule_|cancel_|rollback_)/.test(name))
