...
try {
  ...
  const today = scheduler.parseToCronExpr('hari ini jam 09:00', 'Asia/Jakarta', new Date('2026-09-15T01:51:00.000Z'))
  assert.equal(today.runAt, '2026-09-15T02:00:00.000Z')
  assert.equal(today.runOnce, true)
  assert.equal(scheduler.parseToCronExpr('hari ini jam 08:00', 'Asia/Jakarta', new Date('2026-09-15T01:51:00.000Z')), null)
  const createdReminder = scheduler.createScheduledTask({ userId: 'alice', title: 'Makan', description: 'Cari makan', cronExpr: today.cron, humanSchedule: today.human, runOnce: true, runAt: today.runAt, timezone: 'Asia/Jakarta', idempotencyKey: 'reschedule-test' })
  const movedReminder = scheduler.rescheduleScheduledTask({ userId: 'alice', taskId: createdReminder.id, schedule: '10 menit lagi', timezone: 'Asia/Jakarta' })
  assert.equal(movedReminder.success, true)
  assert.equal(movedReminder.task.id, createdReminder.id)
